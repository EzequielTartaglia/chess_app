...
export default function HomePage() {
  ...
  const SecondaryActionButton = ({ children, onClick, className = "" }) => {
    return (
      <button
        className={`
              w-[95px] h-[32px]
  ...
  const FaqActionButton = ({ children, onClick, className = "" }) => {
    return (
      <button
        className={`
              w-[90px] h-[30.69px] 
    ...
    <div className="relative w-[1920px] h-[1080px] bg-[#101010] overflow-hidden">
      ...
      >
        ...
        <div className="flex justify-between mt-0 gap-2">
          <SecondaryActionButton>TÉRMINOS Y CONDIC</SecondaryActionButton>
          <SecondaryActionButton>MANUAL DE USUARIO</SecondaryActionButton>
          <FaqActionButton>F.A.Qs</FaqActionButton>
        </div>
